...
Release: 改进跨短暂中断的时间连续性判断，并把应用内更新失败时的下载兜底与错误提示做得更清楚。
App note: 改进时间连续性统计，并优化更新失败时的手动下载兜底。
### Added

- 为更新面板和更新弹窗补充更明确的下载进度展示，以及更新失败后的手动下载兜底入口。
- 为 tracking runtime 增加持续参与连续性模型，支持在短暂返回同一应用时保持更稳定的有效时长表现。
### Changed

- 调整时间连续性与有效时长的编译规则，让 History、Dashboard 和相关统计在短暂切换、返回与持续参与场景下更一致。
- 调整应用内更新体验：检查、下载、安装三个阶段的状态与动作更明确，失败后优先给出下一步操作，而不是只显示原始错误。
### Fixed
...
- 移除正常更新状态下多余的“查看发布页”入口，减少设置页中的无效干扰。
### Internal

- 继续收口 tracking 相关的 Rust 边界，补充 `active_session`、`continuity` 等更明确的运行时模块，并把 Windows 媒体/音频能力从更混杂的位置拆分出来。
- 进一步按 `app / features / shared / platform` 与 Rust `app / commands / platform / engine / domain / data` 的长期方向推进 owner-first 收口，减少高吸力层重新变厚的风险。
